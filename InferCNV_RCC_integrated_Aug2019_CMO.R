...
folder_path   = 'TWAN-RCC/inferCNV_integrated/'
...
annot_path    = paste0(folder_path,'integrated_QC_annot.txt')
exp_path      = paste0(folder_path,'integrated_QC_raw_expression.txt')
rds_path      = 'TWAN-RCC/ccRCC_A1_B2_D2_integrated_Aug2019.rds'
h5_path       = "TWAN-RCC/TWAN-RCC-D2/outs/raw_gene_bc_matrices_h5.h5" # Use to convert ensambl ID/Symbol
output_path   = paste0(folder_path,'inferCNV_result')

##################################################################
## Assisting function
## Ref: Create ensl symbol and id ref list
...
exp_df <- GetAssayData(seurat_obj) %>% as.data.frame %>% rownames_to_column(var = 'gene')
...
write.table(Idents(seurat_obj) %>% as.data.frame, annot_path, quote= F, col.names = F, sep = '\t')
...
normal_groups <- c("Astrocyte", "Bcell", 
                   "CD4Tcell", "CD8Tcell", "DC", "Endothelial", "Macrophage", "Mesangial", 
                   "NK(1)", "NK(2)", "NK(3)", "NK(4)", "Pro-Bcell", "Treg") # Replace this with normal cells in the sample
